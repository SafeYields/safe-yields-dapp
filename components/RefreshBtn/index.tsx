...
const Spin: FC<ComponentProps> = ({ spinning, countdown }) => {
  const theme = useMantineTheme();
  return (
    <svg
      style={{
        animationName: spin,
        animationDuration: '696ms',
    ...
    >
      <path
        stroke='none'
        fill={theme.colors.limeGreen[0] + '66'}
...
function RefreshBtn({ trade, onRefresh, loading }: Props) {
  ...
  return (
    <Box className={classes.spinWrapper} role='button' onClick={onRefresh}>
      <Spin spinning={loading} countdown={countdown} />

      {!!countdown && !loading && (
        <Box className={classes.countDown}>{(countdown / 1_000).toFixed()}</Box>
      )}
    </Box>
  );
}
