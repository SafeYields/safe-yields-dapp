...
const Stars = (props: any) => {
  ...
  const rotate = (stars: any, center: any) => {
  ...
  const initBackground = () => {
    ...
    const stars = [];
    const maxRad = Math.sqrt(Math.pow(center.x, 2) + Math.pow(center.y, 2));
  ...
  return (
    <div style={{ backgroundColor: bgColor, height: '100vh', position: 'relative', width: '100%' }}>
      <svg style={{ opacity, position: 'absolute', pointerEvents: 'none', width: '100%', height: '100%' }}
           id='galaxy' ref={galaxy}>
